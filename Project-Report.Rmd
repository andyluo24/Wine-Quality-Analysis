...
```{r results = FALSE}
# Backward Search
wine_mod = lm(quality ~ ., data = training_data)
wine_mod_back_aic = step(wine_mod, direction = "backward")
```
...
```{r results = FALSE}
n = length(resid(wine_mod))
wine_mod_back_bic = step(wine_mod, direction = "backward", k = log(n))
```
...
```{r results = FALSE}
# Forward Search
wine_mod_Start = lm(quality ~ 1, data = training_data)
wine_mod_forw_aic = step(wine_mod_Start, scope = quality ~ fixed.acidity + volatile.acidity + citric.acid + residual.sugar + chlorides + free.sulfur.dioxide + total.sulfur.dioxide + density + pH + sulphates + alcohol + type, direction = "forward")
```
...
```{r results = FALSE}
wine_mod_forw_bic = step(wine_mod_Start, scope = quality ~ fixed.acidity + volatile.acidity + citric.acid + residual.sugar + chlorides + free.sulfur.dioxide + total.sulfur.dioxide + density + pH + sulphates + alcohol + type, direction = "forward", k = log(n))
```
...
```{r}
# comparison
anova(wine_mod_back_aic, wine_mod_back_bic)$"Pr(>F)"[2]
```

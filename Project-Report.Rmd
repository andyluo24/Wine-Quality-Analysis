...
From the above graphs, we find that all those models are not performing well. It's hard to notice a conclusive pattern. To compare these models in a more visualized way, we will compare the residual standard error and R-squared of each model to select the best one.
...
We would like to compare the best simple linear regression model and this full addictive model to find the better of these two models. 
...
summary(wine_mod_exhaustive_quad_aic)
```
...
It looks like there are many redundant insignificant terms in this model. We then use backward selection with AIC to create a new model.
...
We would like to seek out some methods that can improve the performance of **int_aic**. We will conduct some diagnostics to **int_aic** and perform the outliers and influential points diagnostics to the training data. 

...
We will fit some more complex models. We want to start with a big model and use backwards AIC to reduce the number of parameters.   
...
```{r}
two_way_interaction_model = lm(quality ~ .^2, data = training_data)
fit_aic_2 = step(two_way_interaction_model, direction = "backward", trace = 0)
```

```{r}
length(coef(fit_aic_2))
```

```{r}
summary(fit_aic_2)$r.sq
```

```{r}
big_fit = lm(quality ~ . ^ 2 + I(fixed.acidity ^ 2) + I(volatile.acidity ^ 2) + I(residual.sugar ^ 2) + I(chlorides ^ 2) + I(free.sulfur.dioxide ^ 2) + I(total.sulfur.dioxide ^ 2) + I( density ^ 2) + I(pH ^ 2) + I(sulphates ^ 2) + I(alcohol ^ 2), data = training_data)
fit_aic_3 = step(big_fit, direction = "backward", trace = 0)
```

```{r}
length(coef(fit_aic_3))
```

```{r}
summary(fit_aic_3)$r.sq
```
